...
function TeamMemberCard({name,role,imageUrl}) {
    const dispatch = useDispatch();
  return (
    <div className='w-1/2 shadow-lg rounded-lg p-4 flex flex-col gap-2 justify-center items-center'>
            <img src={imageUrl} alt="" className='object-cover overflow-hidden border-2 w-40 h-40 border-blue-300 rounded-full'/>
             <h1 className='text-bold text-xl'>{name}</h1>
             <h1 className='text-lg text-gray-600'>{role}</h1>
